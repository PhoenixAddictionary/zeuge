...
/** The named reason a lock either was, or was not, removed — carried on a `LOCK_HELD` refusal
 *  so a stuck ledger can be diagnosed without guessing which case fired. */
export type LockDecisionReason =
  | "HOLDER_ALIVE"
  | "HOLDER_DEAD" // removable
  | "HOLDER_DEAD_TOO_YOUNG" // dead, but not yet past STALE_AGE_MS
  | "UNKNOWN_LIVENESS" // process.kill errored with something other than ESRCH (e.g. EPERM)
  | "UNREADABLE_LOCK_INFO"; // lock file missing/unparseable/no usable pid

export interface LockDecision {
  removable: boolean;
  reason: LockDecisionReason;
  pid: number | null;
  /** Present only for UNKNOWN_LIVENESS: the errno code process.kill reported. */
  errnoCode?: string;
}

export type AcquireResult =
  | { ok: true; fd: number }
  | { ok: false; reason: "LOCK_HELD"; holderPid: number | null; detail: LockDecisionReason; detailMessage: string };
...
function probeLiveness(pid: number): { liveness: Liveness; errnoCode?: string } {
  try {
    process.kill(pid, 0);
    return { liveness: "ALIVE" }; // no error: signal 0 delivered, process exists (and we have permission)
  } catch (err) {
    const code = (err as NodeJS.ErrnoException).code;
    if (code === "ESRCH") return { liveness: "DEAD" };
...
function assessLock(info: LockInfo | null, now: number): LockDecision {
  if (!info) return { removable: false, reason: "UNREADABLE_LOCK_INFO", pid: null };
...
function decisionMessage(decision: LockDecision): string {
...
function acquireLock(lockPath: string, timeoutMs: number): AcquireResult {
  const deadline = Date.now() + timeoutMs;
  let staleRemovalAttempted = false;
  let lastDecision: LockDecision = { removable: false, reason: "UNREADABLE_LOCK_INFO", pid: null };

  while (true) {
    try {
      const fd = fs.openSync(lockPath, "wx");
      const info: LockInfo = { pid: process.pid, acquired_at: Date.now() };
      try {
...
export class LedgerLockHeldError extends Error {
  readonly code = "LOCK_HELD" as const;
  readonly holderPid: number | null;
  readonly detail: LockDecisionReason;

  constructor(lockPath: string, holderPid: number | null, detail: LockDecisionReason, detailMessage: string) {
...
export function withLedgerLock<T>(ledgerPath: string, fn: () => T, timeoutMs = DEFAULT_TIMEOUT_MS): T {
