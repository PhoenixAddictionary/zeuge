...
export type CommandKind = "test" | "build" | "deploy" | "other";

type Tokens = string[];

function tokenize(cmd: string): Tokens {
...
function stripLeadingNoise(tokens: Tokens): Tokens {
  let i = 0;
...
function resolveDelegatedTokens(tokens: Tokens): Tokens | null {
...
function isScriptInvocation(tokens: Tokens, scriptName: string, opts: { npmBareAliases?: string[]; bareAliasForOthers?: boolean } = {}): boolean {
  if (tokens.length < 2) return false;
  const mgr = programName(tokens[0]);
  if (!NPM_LIKE_MANAGERS.has(mgr)) return false;
  const second = tokens[1];

  if (mgr === "npm") {
    if (opts.npmBareAliases?.includes(second)) return true;
...
function isStandaloneTestProgram(tokens: Tokens): boolean {
  if (tokens.length === 0) return false;
  const prog = programName(tokens[0]);
  if (STANDALONE_TEST_PROGRAMS.has(prog)) return true;
  if (prog === "bundle" && tokens[1] === "exec" && programName(tokens[2]) === "rspec") return true; // bundle exec rspec
  return false;
}

function isNpxTestInvocation(tokens: Tokens): boolean {
  if (tokens.length < 2 || programName(tokens[0]) !== "npx") return false;
...
function isSubcommandTestInvocation(tokens: Tokens): boolean {
  if (tokens.length < 2) return false;
  const prog = programName(tokens[0]);
  if (prog === "node" && tokens[1] === "--test") return true;
  if (prog === "deno" && tokens[1] === "test") return true;
  if (prog === "bun" && tokens[1] === "test") return true;
  if (prog === "cargo" && tokens[1] === "test") return true;
  if (prog === "go" && tokens[1] === "test") return true;
  if (prog === "dotnet" && tokens[1] === "test") return true;
  if (prog === "mvn" && tokens.includes("test")) return true; // e.g. "mvn -B test", "mvn clean test"
  if (prog === "gradlew" && tokens.slice(1).includes("test")) return true; // ./gradlew test, gradlew.bat test
  return false;
}

function isTestInvocation(tokens: Tokens): boolean {
  return (
    isScriptInvocation(tokens, "test", { npmBareAliases: ["test", "t"] }) ||
...
function isBuildInvocation(tokens: Tokens): boolean {
  if (isScriptInvocation(tokens, "build", { npmBareAliases: [] })) return true; // npm: run-only, matches original scope
  const prog = programName(tokens[0]);
  if (prog === "tsc") return true;
  if (prog === "webpack") return true;
  if (prog === "vite" && tokens[1] === "build") return true;
  if (prog === "cargo" && tokens[1] === "build") return true;
  if (prog === "go" && tokens[1] === "build") return true;
  if (prog === "dotnet" && tokens[1] === "build") return true;
  if (prog === "mvn" && tokens.includes("package")) return true;
  if (prog === "gradlew" && tokens.slice(1).includes("build")) return true;
  if (prog === "make" && tokens[1] === "build") return true;
  return false;
}

function isDeployInvocation(tokens: Tokens): boolean {
  if (isScriptInvocation(tokens, "deploy", { npmBareAliases: [], bareAliasForOthers: false })) return true; // npm run deploy only, matches original scope
  const prog = programName(tokens[0]);
  if (prog === "vercel" && tokens[1] === "deploy") return true;
  if (prog === "netlify" && tokens[1] === "deploy") return true;
  if (prog === "git" && tokens[1] === "push" && tokens[2] === "heroku") return true;
  if (prog === "kubectl" && tokens[1] === "apply") return true;
  if (prog === "terraform" && tokens[1] === "apply") return true;
  if (prog === "cdk" && tokens[1] === "deploy") return true;
  if (prog === "serverless" && tokens[1] === "deploy") return true;
  if (prog === "sls" && tokens[1] === "deploy") return true;
  if (prog === "fly" && tokens[1] === "deploy") return true;
  return false;
}
...
export function classifyCommandKind(command: string): CommandKind {
  ...
  if (firstSegment.length === 0) return "other";
  ...
  if (ARGUMENT_CONSUMERS.has(programName(tokens[0]))) return "other";
  ...
  if (isTestInvocation(tokens) || (delegated !== null && isTestInvocation(delegated))) return "test";
  if (isBuildInvocation(tokens) || (delegated !== null && isBuildInvocation(delegated))) return "build";
  if (isDeployInvocation(tokens) || (delegated !== null && isDeployInvocation(delegated))) return "deploy";
  return "other";
}
