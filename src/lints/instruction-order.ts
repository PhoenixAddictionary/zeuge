...
export type ClauseClass = "ORDER" | "STANDING" | "ACTION" | "OTHER";

export interface InstructionOrderFinding {
  actionLine: number; // 1-based
  orderLine: number; // 1-based
  action: string;
  order: string;
  blockStart: number; // 1-based
...
const STANDING: RegExp[] = [/nie /, /niemals/, /never /, /must not/, /darf nicht/, /do not /, /kein[e]? /];
...
export function getClauseClass(line: string): ClauseClass {
  let t = line.trim().replace(/^[-*]\s+/, "").replace(/^\d+\.\s+/, "");
  t = t.replace(/\*\*/g, "");
  if (!t) return "OTHER";
  const low = t.toLowerCase();

  for (const m of ORDER_MARKERS) if (m.test(low)) return "ORDER";
  for (const m of STANDING) if (m.test(low)) return "STANDING";
  for (const v of ACTION_VERB_PATTERNS) if (v.test(low)) return "ACTION";
  return "OTHER";
}
 ...
 */
export function getFindingsLoose(lines: string[]): InstructionOrderFinding[] {
  const findings: InstructionOrderFinding[] = [];
  let blockStart = 0;
...
const TIGHT_ORDER_RE =
  /^\s*(?:[-*]|\d+[.)])\s+(?:\*\*|__|\*|_|`)?\s*(zuerst|als erstes|bevor|vorher|ausser wenn|ausser bei|es sei denn|nur wenn|first|firstly|before|unless|only if|only when|except when)\b/i;
 ...
 */
export function getFindingsTight(lines: string[]): InstructionOrderFinding[] {
  const findings: InstructionOrderFinding[] = [];
  let blockStart = 0;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!TIGHT_ORDER_RE.test(line)) continue;

    const prevIdx = i - 1;
    if (prevIdx < blockStart) continue; // nothing precedes this bullet in the current block
    const prevLine = lines[prevIdx];
    if (prevLine === undefined || prevLine.trim() === "") continue; // a blank line breaks adjacency
    if (!TIGHT_BULLET_RE.test(prevLine)) continue; // preceding line is not itself a bullet
    if (getClauseClass(prevLine) !== "ACTION") continue; // preceding bullet is not an unconditional action
...
export function scanFile(filePath: string): InstructionOrderFinding[] {
...
export const FIELD_TEST_NOTE =
  "heuristic: 12 findings over 696 public instruction files in the 2026-09-11 field test; expect false positives";
