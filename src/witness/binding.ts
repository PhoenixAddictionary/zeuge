...
export const PLACEHOLDER_SESSION = "session:00000000000000000000000000000000";
...
export function isUnknownSession(sessionId: string | undefined): boolean {
  return sessionId === undefined || sessionId === "" || sessionId === PLACEHOLDER_SESSION;
}
...
export const WITNESS_WINDOW_MS = 30 * 60 * 1000;
...
export function withinWitnessWindow(claim: ClaimRecord, witness: Witness): boolean {
  const claimTime = Date.parse(claim.occurred_at);
  const witnessTime = Date.parse(witness.observed_at);
  if (!Number.isFinite(claimTime) || !Number.isFinite(witnessTime)) return false;
  return Math.abs(claimTime - witnessTime) <= WITNESS_WINDOW_MS;
}
...
export function witnessBacksClaim(witness: Witness, claim: ClaimRecord): boolean {
  if (!witness.binds.includes(claim.claim_type)) return false;
  const bothKnown = !isUnknownSession(witness.session_id) && !isUnknownSession(claim.session_id);
  if (bothKnown && witness.session_id !== claim.session_id) return false;
